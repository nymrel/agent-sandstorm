...
export class StepLimitExceededError extends Error {
  ...
  constructor(message: string, totalSteps: number, maxSteps: number) {
    super(message);
  ...
  public recordAction(actionIdentifier: string, detail?: string): void {
    ...
    if (this.totalSteps > this.maxSteps) {
      throw new StepLimitExceededError(
        `Sandstorm Execution Brake: Max step limit (${this.maxSteps}) exceeded. Halting agent.`,
        this.totalSteps,
        this.maxSteps
      );
    }
  ...
  private checkLoops(): void {
    ...
    if (consecutiveCount >= this.loopThreshold) {
      throw new RunawayLoopError(
        `Sandstorm Emergency Brake: Runaway recursive loop detected! Action '${lastAction}' repeated ${consecutiveCount} consecutive times.`,
        lastAction,
        consecutiveCount,
        this.totalSteps
      );
    ...
    for (let period = 2; period <= 5; period++) {
      ...
      if (len >= requiredHistory) {
        ...
        if (cycleMatches >= this.loopThreshold) {
          ...
          throw new RunawayLoopError(
            `Sandstorm Emergency Brake: Runaway cyclic loop detected! Pattern [${patternStr}] repeated ${cycleMatches} cycles.`,
            patternStr,
            cycleMatches,
            this.totalSteps
          );
